...
- `firmware/drivers/waveshare/` derives from Waveshare board-support source. Retain the MIT notices present in those files. In particular, `firmware/drivers/waveshare/qspi/qspi.pio` is derived from the Waveshare Team RP2350 Touch AMOLED board support dated 2025-03-20; its corresponding MIT permission text is retained in the adjacent `qspi_pio.c` source.
...
- `firmware/pico_sdk_import.cmake` and the adapted `firmware/boards/waveshare_rp2350_touch_amoled_2.41.h` board header originate from Raspberry Pi Pico SDK material and retain BSD-3-Clause notices in their file headers. The Pico SDK itself is not included in this repository.
